...
Date labels use IST and a documented fixed demo clock. ICS export uses UTC timestamps. Fonts ship with the application. `/api/analyze` supports optional server-side Gemini structured output and a conservative local fallback; `/api/query` optionally explains deterministic decision facts. Provider keys stay server-side. Resume files never leave the browser.

`lib/intelligence.ts` owns weighted impact, dependency state, focus selection and certification simulations. `lib/analyzer.ts` validates extraction and action dependencies. `lib/query.ts` handles narrow assistant intents. `lib/server-ai.ts` calls Gemini with JSON Schema structured output and bounded retries for temporary capacity errors. Focus selection protects urgent ready actions and greedily fits the budget; unknown eligibility cannot unlock workflow steps. Undoing a prerequisite removes completed descendants.
